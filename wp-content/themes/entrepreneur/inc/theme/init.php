...
class MP_Entrepreneur {
	...
	public function setup() {
		...
		load_theme_textdomain( 'entrepreneur', get_template_directory() . '/languages' );
	...
	function widgets_init() {
		...
		register_sidebar( array(
			'name'          => __( 'First Section Widget Area', 
				'entrepreneur'  ),
			'id'            => 'sidebar-first-section',
			'description'   => __( 'Appears on front page.', 
				'entrepreneur'  ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h2 class="widget-title">',
		...
		register_sidebar( array(
			'name'          => __( 'Services & Prices Widget Area', 
				'entrepreneur'  ),
			'id'            => 'sidebar-services',
			'description'   => __( 'Appears on front page.', 
				'entrepreneur'  ),
	...
	function new_excerpt_more( $more ) {
		...
		return '... <p><a class="more-link" href="' . get_permalink( $post->ID ) . '">' . __( 'read more', 
			'entrepreneur'  ) . '</a></p>';
	}
	...
	function theme_requires() {
		...
		*/
		 ...
		 * Theme Updater
		 */
		//require get_template_directory() . '/inc/theme/theme-updater.php';
	...
	function admin_enqueue_scripts() {
		if ( is_callable( 'is_customize_preview' ) && is_customize_preview() ) {
			wp_enqueue_script( $this->get_prefix() . 'sections', get_template_directory_uri() . '/js/theme-sections.min.js', '', $this->get_theme_vertion(), true );
		}
	}
	...
	public function wizard_admin_notice() {
		...
		if ( $isThemeActivation && ! get_user_meta( get_current_user_id(), $this->get_prefix() . 'wizard_dismiss', true ) ) :
			...
			<div class="notice notice-success is-dismissible">
				<p>
					<strong><?php _e( 'You&#8217;ve installed Entrepreneur theme. Click &#34;Run Theme Wizard&#34; to view a quick guided tour of theme functionality and complete the installation.',  
							'entrepreneur'  ); ?></strong>
				</p>
				<p>
					<a class="button button-primary"
					   href="<?php echo esc_url( admin_url( 'themes.php?page=theme-setup' . '&' . $this->get_prefix() . 'wizard-dismiss=dismiss_admin_notices' ) ); ?>"><strong><?php _e( 'Run Theme Wizard',  
								'entrepreneur'  ); ?>
						</strong></a>
					<a class="button"
					   href="<?php echo esc_url( admin_url( 'themes.php'  . '?' . $this->get_prefix() . 'wizard-dismiss=dismiss_admin_notices') ); ?>"
					   class="dismiss-notice"
					   target="_parent"><strong><?php _e( 'Skip',  
								'entrepreneur'  ); ?>
						</strong></a>
				</p>
			</div>
