	...
	if ( get_theme_mod( mp_entrepreneur_get_prefix() . 'show_categories', '1' ) === '1' || get_theme_mod( mp_entrepreneur_get_prefix() . 'show_categories' ) ):
		if ( strcmp( get_post_type( $mp_entrepreneur_post ), 'post' ) === 0 ) :
			...
			if ( ! empty( $mp_entrepreneur_categories ) ) { ?>
				<span><i class="fa fa-folder-open"></i><?php _e( 'Posted in', 
						'entrepreneur'  ); ?></span>
				<?php
				foreach ( $mp_entrepreneur_categories as $mp_entrepreneur_category ) {
					...
					echo '<a href="' . esc_url( get_category_link( $mp_entrepreneur_category->term_id ) ) . '"' . $mp_entrepreneur_class . ' alt="' . esc_attr( sprintf( __( 'View all posts in %s',  
							'entrepreneur'  ), $mp_entrepreneur_category->name ) ) . '">' . esc_html( $mp_entrepreneur_category->name ) . '</a>' . $mp_entrepreneur_separator;
				}
			}
		endif;
	endif;
	...
	if ( get_theme_mod( mp_entrepreneur_get_prefix() . 'show_tags', '1' ) === '1' || get_theme_mod( mp_entrepreneur_get_prefix() . 'show_tags' ) ):
		if ( strcmp( get_post_type( $mp_entrepreneur_post ), 'post' ) === 0 ) :
			...
			if ( ! empty( $mp_entrepreneur_tags ) ) { ?>
				<span><i class="fa fa-tags"></i><?php _e( 'Tagged with', 
						'entrepreneur'  ); ?></span>
				<?php
				foreach ( $mp_entrepreneur_tags as $mp_entrepreneur_tag ) {
					...
					echo '<a href="' . esc_url( get_tag_link( $mp_entrepreneur_tag->term_id ) ) . '"' . $mp_entrepreneur_class . ' alt="' . esc_attr( sprintf( __( 'View all posts in %s',  
							'entrepreneur'  ), $mp_entrepreneur_tag->name ) ) . '">' . esc_html( $mp_entrepreneur_tag->name ) . '</a>' . $mp_entrepreneur_separator;
				}
			}
		endif;
	endif;
...
function mp_entrepreneur_post_meta( $mp_entrepreneur_post ) {
	if ( get_theme_mod( mp_entrepreneur_get_prefix() . 'show_meta', '1' ) === '1' || get_theme_mod( mp_entrepreneur_get_prefix() . 'show_meta' ) || get_theme_mod( mp_entrepreneur_get_prefix() . 'show_tags', '1' ) === '1' || get_theme_mod( mp_entrepreneur_get_prefix() . 'show_tags' ) || get_theme_mod( mp_entrepreneur_get_prefix() . 'show_categories', '1' ) === '1' || get_theme_mod( mp_entrepreneur_get_prefix() . 'show_categories' ) ):
		...
		<div class="entry-meta">
			...
			<span class="author"><i
					class="fa fa-user"></i><?php _e( 'Posted by', 
					'entrepreneur'  ); ?> </span><?php the_author_posts_link(); ?>
			<?php if ( comments_open() ) : ?>
				<a class="last comments-meta " href="<?php
				if ( ! is_single() ): the_permalink();
				endif;
				?>#comments"><i
						class="fa fa-comment"></i><span><?php comments_number( '0 ', '1 ', '% ' ); ?><?php _e( 'Comments', 
							'entrepreneur'  ); ?></span></a>
				<?php
			endif;
			mp_entrepreneur_post_tag( $mp_entrepreneur_post );
			mp_entrepreneur_post_category( $mp_entrepreneur_post );
			edit_post_link( '<span>' . __( 'Edit', 
					'entrepreneur'  ) . '</span>', '', '', '', 'fa fa-pencil post-edit-link' );
			?>
		</div>
	<?php endif; ?>
		<?php
...
function mp_entrepreneur_comment( $mp_entrepreneur_comment, $mp_entrepreneur_args, $mp_entrepreneur_depth ) {
	...
	<div class="comment-meta commentmetadata date-post">
		<?php
		printf( __( '<i class="fa fa-calendar"></i><span>%1$s</span>', 
			'entrepreneur'  ), get_comment_date( 'F j, Y' ) );
		?>
...
function mp_entrepreneur_get_post_images( $mp_entrepreneur_post_id = null, $mp_entrepreneur_limit = 1 ) {
	...
	for ( $i = 0; $i < $mp_entrepreneur_limit; $i ++ ) {
		...
		$query              = "SELECT ID FROM {$wpdb->posts} WHERE guid='$mp_entrepreneur_image_src'";
		$mp_entrepreneur_id = $wpdb->get_var( $query );

		if ( ! $mp_entrepreneur_id ) {
			$mp_entrepreneur_result[] = $mp_entrepreneur_image_src;
		} else {
			$mp_entrepreneur_result[] = (int) $mp_entrepreneur_id;
		}
...
function mp_entrepreneur_post_format_image( $mp_entrepreneur_post, $mp_entrepreneur_page_template ) {
	$mp_entrepreneur_img = mp_entrepreneur_get_post_images();
	if ( ! $mp_entrepreneur_img || ! empty( $mp_entrepreneur_img[0] ) ) { ?>
		<div class="entry-thumbnail">
			<?php if ( $mp_entrepreneur_page_template == 'single.php' ): ?>
				<img src="<?php echo $mp_entrepreneur_img[0]; ?>" class="attachment-post-thumbnail wp-post-image"
				     alt="<?php the_title(); ?>">
			<?php else: ?>
				<a href="<?php the_permalink(); ?>"><img src="<?php echo $mp_entrepreneur_img[0]; ?>"
...
function mp_entrepreneur_get_post_format_quote( $mp_entrepreneur_post = null ) {
	...
	$mp_entrepreneur_quote = $mp_entrepreneur_quote . '<p><a href="' . get_permalink() . '" class="more-link">' . __( 'read more',
			'entrepreneur'  ) . '</a></p>';
...
/**
 * Returns the short post content.
 *
 * @since Entrepreneur 1.0.0
 *
 * @param  int $mp_entrepreneur_contentLength .
 *
 */
function mp_entrepreneur_get_content_theme( $mp_entrepreneur_contentLength ) {
	$mp_entrepreneur_content = apply_filters( 'the_content', get_the_content() );
	$mp_entrepreneur_content = wp_strip_all_tags( $mp_entrepreneur_content );
	$mp_entrepreneur_content = wp_kses( $mp_entrepreneur_content, array() );
	$mp_entrepreneur_content = preg_replace( '/<(script|style)(.*?)>(.*?)<\/(script|style)>/is', '', $mp_entrepreneur_content );
	if ( strlen( $mp_entrepreneur_content ) > $mp_entrepreneur_contentLength ) {
		$mp_entrepreneur_content = extension_loaded( 'mbstring' ) ? mb_substr( $mp_entrepreneur_content, 0, $mp_entrepreneur_contentLength ) . '...' : substr( $mp_entrepreneur_content, 0, $mp_entrepreneur_contentLength ) . '...';
	}
	echo $mp_entrepreneur_content;
	?>
	<?php
}
