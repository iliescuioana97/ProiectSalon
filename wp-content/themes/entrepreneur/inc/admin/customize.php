...
class MP_Entrepreneur_Customizer {
	...
	function customize_register( $wp_customize ) {
		...
		$wp_customize->add_setting( $this->get_prefix() . 'copyright', array(
			'sanitize_callback' => array( $this, 'sanitize_text' ),
			'default'           => __( '&#8226; Designed by <a href="http://www.getmotopress.com/" rel="nofollow" title="Premium WordPress Plugins and Themes">MotoPress</a> &#8226; Proudly Powered by <a href="http://wordpress.org/" rel="nofollow" title="Semantic Personal Publishing Platform">WordPress</a>', 'entrepreneur' ),
			'capability'        => 'edit_theme_options',
			'transport'         => 'postMessage'
		) );
		$wp_customize->add_control( new MP_Entrepreneur_Customize_Textarea_Control( $wp_customize, $this->get_prefix() . 'copyright', array(
			'label'    => __( '"Designed and Powered..." text in footer', 'entrepreneur' ),
			'section'  => $this->get_prefix() . 'general_section',
			'settings' => $this->get_prefix() . 'copyright',
			'priority' => 2
		) ) );
		...
		/*
		 * Add the upload control for the 'logo' setting.
		 */
		...
		$wp_customize->add_section(
			$this->get_prefix() . 'posts_settings', array(
				'title'      => esc_html__( 'Posts Settings', 
					'entrepreneur'  ),
				'priority'   => 150,
				'capability' => 'edit_theme_options'
			)
		...
		$wp_customize->add_section(
			$this->get_prefix() . 'first_section', array(
				...
				'priority'    => 80,
				'capability'  => 'edit_theme_options',
				'description' => __( '<i>Fill in this section by adding widgets to "Customize > Widgets > First section"</i><hr/>', 
					'entrepreneur'  )
			)
		);
		/*
		...
		$wp_customize->add_control(
			new WP_Customize_Control(
				...
				'label'    => esc_html__( 'Hide this section', 
					'entrepreneur'  ),
     ...
     */
		...
		*/
		...
		$wp_customize->add_section(
			$this->get_prefix() . 'news_section', array(
				...
				'priority'    => 88,
				'capability'  => 'edit_theme_options',
				'description' => __( '<i>Automatically filled by your latest posts.</i><hr/>', 
					'entrepreneur'  )
			)
		);
		...
		$wp_customize->add_setting( $this->get_prefix() . 'news_subtitle', array(
			'sanitize_callback' => array( $this, 'sanitize_text' ),
			'default'           => __( 'Updates from the blog', 
				'entrepreneur'  ),
			'capability'        => 'edit_theme_options',
			'transport'         => 'postMessage'
		) );
		...
		*/
		...
		$wp_customize->add_setting( $this->get_prefix() . 'call_to_action_title', array(
			'sanitize_callback' => array( $this, 'sanitize_text' ),
			'default'           => __( 'Book your visit online and save 10&#37;', 
				'entrepreneur'  ),
			'capability'        => 'edit_theme_options',
			'transport'         => 'postMessage'
		) );
