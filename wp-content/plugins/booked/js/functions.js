...
function services_change_employees_list() {
    var labelServiciu = jQuery("label:contains('Serviciu')");
    var inputServiciu = jQuery(labelServiciu).next().next();

    var labelAngajat = jQuery("label:contains('Angajat')");
    var inputAngajat = jQuery(labelAngajat).next().next();
    
    if (typeof inputServiciu === "undefined")
    {
        console.log("nu s-a gasit input");
        return false;
    }
    
    console.log('am gasit inptul de servicii');
    
    getServiciiList(inputServiciu);    
    
    inputServiciu.on('change', function () {
        var idserviciu = jQuery(inputServiciu).val();
        jQuery.ajax({
            method: "GET",
...
function getServiciiList(inputServicii) {
    jQuery.ajax({
        method: "GET",
        url: '/wp-admin/admin-ajax.php',
        data: {action: "preluare_servicii"}

    })
            .done(function (msg) {

                var $el = inputServicii;
                $el.empty(); // remove old options

                if (msg.status === 'success') {
                    $el.append(jQuery("<option></option>")
                            .attr("value", '').text('Alege un serviciu'));
                    jQuery.each(msg.services, function (key, value) {
                    ...
                    $el.append(jQuery("<option></option>")
                            .attr("value", '').text('Nu exista servicii disponibile'));
                }
