...
// END public static function activate
        ...
        public function booked_new_addons() {
            ...
            if (!$json_array): $json_array = array();
            else : $json_array = json_decode($json_array, true);
            endif;
            if (!empty($json_array)):
        ...
        public function booked_add_new_addons_bubble() {
            ...
            foreach ($submenu as $key => $value) :
                if ($key == 'booked-appointments') :
                    if ($new_addons) {
                        $submenu[$key][4][0] .= " <span style='position:relative; top:1px; margin:-2px 0 0 2px' class='update-plugins count-$new_addons' title='$new_addons'><span style='padding:0 6px 0 4px; min-width:7px; text-align:center;' class='update-count'>" . $new_addons . "</span></span>";
                    }
                    return;
                endif;
            endforeach;
        ...
        public function booked_add_pending_appt_bubble() {
            ...
            foreach ($submenu as $key => $value) :
                if ($key == 'booked-appointments') :
                    if ($pending) {
                        $submenu[$key][1][0] .= " <span style='position:relative; top:1px; margin:-2px 0 0 2px' class='update-plugins count-$pending' title='$pending'><span style='padding:0 6px 0 4px; min-width:7px; text-align:center;' class='update-count'>" . $pending . "</span></span>";
                    }
                    return;
                endif;
            endforeach;
...
function preluareListaAngajati() {
    ...
    $EmplServTable = $wpdb->prefix . "services_staff";
    $EmployeeTable = $wpdb->prefix . "staff";
...
function preluareListaServicii() {
    global $wpdb;

    $ServicesTable = $wpdb->prefix . "services";

    $sql = "SELECT * FROM `$ServicesTable` AS e ";

    $allServices = $wpdb->get_results($wpdb->prepare($sql));

    if (!$allServices) {
        $response = array(
            'status' => 'error',
            'message' => 'Nu exista servicii!',
            'services' => array(),
        );
    } else {
        $response = array(
            'status' => 'success',
            'message' => 'Am gasit servicii!',
...
add_action('wp_ajax_nopriv_preluare_servicii', 'preluareListaServicii');
add_action('wp_ajax_preluare_servicii', 'preluareListaServicii');
